...
def month_info(m):  # get user input for month (all, january, february, ... , june)
    if m == 'month':
        ...
        while month.strip().lower() not in ['january', 'february', 'march', 'april', 'may', 'june']:
            month = input(
                '\nPlease choose between January, February, March, April, May, or June? Please type the full month '
                'name.\n')
...
def ride_duration(df):
    ...
    # df - dataframe returned from time_filters
    print(
        '\n *** What was the total traveling done for 2017 through June, and what was the average time spent on '
        'each trip? ***')
    ...
    print("\n\n The average travel time on 2017 through June was " + avg_days + " days \n")
...
def birth_years(df):
    ...
    try:
        ...
        print("\nThe most frequent year of birth is " + str(most_frequent) + "\n")
...
def disp_raw_data(df):
    ...
    while True:
        ...
        see_data = input(
            "\nWould you like to see five more rows of the data used to compute the stats? Please write 'yes' or "
            "'no' \n").lower()
